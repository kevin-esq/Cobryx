...
public class ReconciliationEngineTests
{
    ...
    public ReconciliationEngineTests()
    {
        ...
        _engine = new ReconciliationEngine(_dbContext, _stripeMock.Object, postingEngine, metrics, clockMock.Object, loggerMock.Object);
    ...
    public async Task ReconcileAsync_ShouldBeSynced_WhenLedgerMatchesStripe()
    {
        var from = DateTime.UtcNow.AddHours(-1);
        var to = DateTime.UtcNow;
        var intentId = "pi_synced_456"; // Use unique ID to avoid conflicts

        // Create ledger account for entries
        var cashAccount = new LedgerAccount(_tenantId, "1010", "Cash", LedgerAccountType.Asset);
        var revenueAccount = new LedgerAccount(_tenantId, "4000", "Revenue", LedgerAccountType.Revenue);
        _dbContext.LedgerAccounts.AddRange(cashAccount, revenueAccount);
        await _dbContext.SaveChangesAsync();

        // Create transaction with entries that match Stripe amount ($100.00 = 10000 cents)
        var tx = new LedgerTransaction(_tenantId, "Sync Test", $"PAY-STRIPE-{intentId}");
        tx.AddEntry(cashAccount.Id, 100.00m, 0); // Debit cash
        tx.AddEntry(revenueAccount.Id, 0, 100.00m); // Credit revenue
        _dbContext.LedgerTransactions.Add(tx);
        await _dbContext.SaveChangesAsync();

        // Detach and re-query to ensure Include works
