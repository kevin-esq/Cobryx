...
public class ReplayEngineTests
{
    private static void SetupDbMock(Mock<ICobryxDbContext> dbMock)
    {
        ...
        dbMock.Setup(d => d.ShadowDriftEvents).Returns(new Mock<DbSet<Domain.Decision.ShadowDriftEvent>>().Object);
    }
    ...
    public async Task Replay_WithIdenticalSnapshot_MustGuaranteeAbsoluteTraceDeterminism()
    {
        ...
        snapshotStoreMock.Setup(x => x.QueueSnapshotAsync(
            ...
            .Callback<Guid, string, string, object, object, object, ExecutionTrace, DecisionResult, bool, CancellationToken>(
                (cid, ver, hash, feat, macroState, port, trace, res, full, ct) => 
                {
        ...
        var ctx = new DecisionContext { Credit = new(), Pricing = new(), Fraud = new() };
        await service.EvaluateAsync(Guid.NewGuid(), ctx, overrideSeed: 12345);

        var replayEngine = new ReplayEngine(service, new Mock<ILogger<ReplayEngine>>().Object);
        var result = await replayEngine.ReplayAsync(new ReplaySnapshotAdapter(snapshots.Last()));
    ...
    public async Task DecisionService_ShadowMode_ShouldTriggerAsyncMonitoring()
    {
        ...
        serviceProviderMock.Setup(x => x.GetService(typeof(Application.Decision.Interfaces.IReplayEngine))).Returns(replayEngineMock.Object);
        serviceProviderMock.Setup(x => x.GetService(typeof(Application.Decision.Interfaces.IShadowMonitor))).Returns(shadowMonitorMock.Object);
        ...
        var ctx = new DecisionContext { Credit = new(), Pricing = new(), Fraud = new() };
        var result = await service.EvaluateAsync(Guid.NewGuid(), ctx);
