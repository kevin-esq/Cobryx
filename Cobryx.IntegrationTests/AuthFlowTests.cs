...
public class AuthFlowTests : IClassFixture<CobryxWebApplicationFactory>, IAsyncLifetime
{
    ...
    private string GetUniqueEmail() => $"user_{Guid.NewGuid():N}@example.com";
    ...
    public async Task HappyPath_SignupToOnboard_Success()
    {
        ...
        var signupResp = await _client.PostAsJsonAsync("/api/v1/auth/signup", command);
        ...
        var verifyResp = await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token));
        ...
        var onboardResp = await _client.PostAsJsonAsync("/api/v1/auth/onboard", onboardReq);
    ...
    public async Task Login_WithoutEmailVerification_Returns401WithCollapsedError()
    {
        ...
        await _client.PostAsJsonAsync("/api/v1/auth/signup", command);
    ...
    {
        var email = GetUniqueEmail();
        await _client.PostAsJsonAsync("/api/v1/auth/signup", new SignUpCommand("InvPass Corp", "Inv", "Pass", email, DefaultPassword));
        var token = _emailService.GetLastToken(email);
        await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token!));

        var loginResp = await _client.PostAsJsonAsync("/api/v1/auth/login", new LoginCommand(email, "WrongPass123!"));
        loginResp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    ...
    public async Task RefreshToken_ReuseRevoked_InvalidatesSessionFamily()
    {
        ...
        await _client.PostAsJsonAsync("/api/v1/auth/signup", new SignUpCommand("Reuse Corp", "Reuse", "User", email, DefaultPassword));
        var token = _emailService.GetLastToken(email);
        await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token!));
    ...
    {
        var email = GetUniqueEmail();
        await _client.PostAsJsonAsync("/api/v1/auth/signup", new SignUpCommand("Fail Corp", "Fail", "User", email, DefaultPassword));
        var token = _emailService.GetLastToken(email);
        await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token!));
    ...
    {
        var email = GetUniqueEmail();
        await _client.PostAsJsonAsync("/api/v1/auth/signup", new SignUpCommand("Dual Corp", "Dual", "User", email, DefaultPassword));
        var token = _emailService.GetLastToken(email);
        await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token!));
        var loginResp = await _client.PostAsJsonAsync("/api/v1/auth/login", new LoginCommand(email, DefaultPassword));
        var loginResult = await loginResp.Content.ReadFromJsonAsync<Cobryx.Api.Contracts.V1.Common.ApiSuccessResponse<AuthResponseContract>>();
        ...
        await _client.PostAsJsonAsync("/api/v1/auth/onboard", onboardReq);

        var onboardResp2 = await _client.PostAsJsonAsync("/api/v1/auth/onboard", onboardReq);
    ...
    {
        var email = GetUniqueEmail();
        await _client.PostAsJsonAsync("/api/v1/auth/signup", new SignUpCommand("Logout Corp", "Logout", "User", email, DefaultPassword));
        var token = _emailService.GetLastToken(email);
        await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token!));
        var loginResp = await _client.PostAsJsonAsync("/api/v1/auth/login", new LoginCommand(email, DefaultPassword));
        var loginResult = await loginResp.Content.ReadFromJsonAsync<Cobryx.Api.Contracts.V1.Common.ApiSuccessResponse<AuthResponseContract>>();
    ...
    {
        var resendResp = await _client.PostAsJsonAsync("/api/v1/auth/resend-verification", new ResendVerificationCommand("nonexistent@example.com", "MOCK_CAPTCHA_TOKEN"));
    ...
    public async Task ResendVerification_AlreadyVerified_Returns200()
    {
        ...
        await _client.PostAsJsonAsync("/api/v1/auth/signup", new SignUpCommand("Verified Corp", "Resend", "User", email, DefaultPassword));
        var token = _emailService.GetLastToken(email);
        await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token!));

        var resendResp = await _client.PostAsJsonAsync("/api/v1/auth/resend-verification", new ResendVerificationCommand(email, "MOCK_CAPTCHA_TOKEN"));
        resendResp.StatusCode.Should().Be(HttpStatusCode.OK);
        var result = await resendResp.Content.ReadFromJsonAsync<Cobryx.Api.Contracts.V1.Common.ApiSuccessResponse>();
        result!.OutcomeCode.Should().Be(AuthOutcomes.VerificationEmailSent);
    }
    ...
    public async Task ResendVerification_TooFast_Returns429()
    {
        ...
        await _client.PostAsJsonAsync("/api/v1/auth/signup", new SignUpCommand("Fast Corp", "Resend", "User", email, DefaultPassword));

        var resp = await _client.PostAsJsonAsync("/api/v1/auth/resend-verification", new ResendVerificationCommand(email, "MOCK_CAPTCHA_TOKEN"));
    ...
    public async Task ResendVerification_InvalidatesOldToken()
    {
        ...
        await _client.PostAsJsonAsync("/api/v1/auth/signup", new SignUpCommand("OldToken Corp", "Resend", "User", email, DefaultPassword));
        ...
        var verifyResp = await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token1!));
        verifyResp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);

        var verifyResp2 = await _client.PostAsJsonAsync("/api/v1/auth/verify-email", new VerifyEmailCommand(token2!));
