...
### UI Translation Protocol

Ensure that outcome codes are extremely clear for frontend consumption.

- **Semantic Clarity:** Codes like `CREDIT.LOAN.INSUFFICIENT_FUNDS` must not require the frontend developer to "guess" the business meaning.
- **Mapping:** The frontend maps these codes directly to localized, human-readable messages (e.g., "No tienes saldo suficiente"). Never return human-readable error messages for the frontend in the API response variables; only codes and technical traces.
...
## 7. Financial Simulation Endpoints

Financial Transparency is a legal standard.

- **Quote Simulation:** Create and document dedicated endpoints for "Simulación de Cuotas" (Installment Simulation).
- **State Mutability:** These endpoints MUST NOT save anything to the database. They exist solely to allow business agents to project financial terms accurately (e.g., "Si me pagas en 12 meses, te queda de a tanto").
- **Naming Convention:** Simulation endpoints should generally follow a `POST /simulate` or `GET /quote` pattern to differentiate from actual contract generation.
