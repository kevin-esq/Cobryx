...
namespace Cobryx.Integration.Tests.Lending;
...
public class ReconciliationAtomicityTests(CobryxWebApplicationFactory factory) : IClassFixture<CobryxWebApplicationFactory>
{
    ...
    [Fact(Skip = "Mock constructor mismatch - FinancialStateEngine changed")]
    public async Task HandlePaymentSuccess_ShouldRollbackAll_WhenStateUpdateFails()
    {
        ...
        var reconService = new PaymentLinkReconciliationService(context, postingEngine, stripeService, mockStateEngine.Object, clock, logger);
        ...
        var link = new PaymentLink(tenantId, customerId, new Money(1000, "MXN"), "token_123", now.AddDays(1), "secret_123", now, loan.Id, "Ref-123");
        link.MarkAsProcessing(paymentIntentId, now);
    ...
    public async Task RecoverStuckLinks_ShouldReconcileSuccessfulIntents()
    {
        ...
        var clock = scope.ServiceProvider.GetRequiredService<IClock>();
        var reconService = new PaymentLinkReconciliationService(context, postingEngine, mockStripe.Object, stateEngine, clock, logger);
        ...
        DateTime now = DateTime.UtcNow;
        var link = new PaymentLink(tenantId, customerId, new Money(500, "MXN"), "token_stuck", now.AddDays(1), "secret_stuck", now, loan.Id, "Ref-Stuck");

        link.MarkAsProcessing(paymentIntentId, now);
        typeof(BaseEntity).GetProperty("UpdatedAt")!.SetValue(link, now.AddHours(-2));
