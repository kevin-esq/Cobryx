...
public static class HealthCheckExtensions
{
    public static IServiceCollection AddCobryxHealthChecks(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        var s3AccessKey  = configuration["Storage:S3:AccessKey"]   ?? throw new InvalidOperationException("S3 AccessKey is missing.");
        var s3SecretKey  = configuration["Storage:S3:SecretKey"]   ?? throw new InvalidOperationException("S3 SecretKey is missing.");
        var s3ServiceUrl = configuration["Storage:S3:ServiceUrl"]  ?? throw new InvalidOperationException("S3 ServiceUrl is missing.");
        var bucketName   = configuration["Storage:S3:BucketName"]  ?? throw new InvalidOperationException("S3 BucketName is missing.");
