...
namespace Cobryx.Application.Common.Observability
{
    public sealed class CobryxMetrics : IDisposable
    {
        ...
        public Counter<long> ShadowExecutionTotal { get; }
        public Counter<long> ShadowDriftedTotal { get; }
        public Counter<long> ShadowCriticalDriftTotal { get; }
        public Counter<long> ShadowTraceMismatchTotal { get; }
        public Counter<long> ShadowNonComparableTotal { get; }
        ...
        private static Func<long> _activeWorkersProvider = static () => 0;
        private static Func<long> _queueLengthProvider = static () => 0;
        private static Func<long> _failedJobsProvider = static () => 0;
        private static Func<long> _deletedJobsProvider = static () => 0;
        private static Func<double> _revenueConcentrationProvider = static () => 0;

        private static Func<double> _wraparoundRiskProvider = static () => 0;
        private static Func<double> _walSyncProvider = static () => 0;
        private static Func<double> _deadTupleRatioProvider = static () => 0;
        private static Func<double> _cacheHitRatioProvider = static () => 0;
        private static Func<double> _waitToWorkRatioProvider = static () => 0;
        ...
        public static void RegisterRevenueConcentrationProvider(Func<double> provider) => _revenueConcentrationProvider = provider;
        ...
        public CobryxMetrics()
        {
            ...
            HangfireActiveWorkers = _meter.CreateObservableGauge("hangfire_active_workers",
                static () => _activeWorkersProvider(), description: "Number of active Hangfire workers");

            HangfireQueueLength = _meter.CreateObservableGauge("hangfire_queue_length",
                static () => _queueLengthProvider(), description: "Number of jobs waiting in Hangfire queues");

            HangfireFailedJobs = _meter.CreateObservableGauge("hangfire_failed_jobs_total",
                static () => _failedJobsProvider(), description: "Total number of failed background jobs");

            HangfireDeletedJobs = _meter.CreateObservableGauge("hangfire_deleted_jobs_total",
                static () => _deletedJobsProvider(), description: "Total number of deleted background jobs");
            ...
            RevenueConcentration = _meter.CreateObservableGauge("cobryx_revenue_concentration_percent",
                static () => _revenueConcentrationProvider(), unit: "%",
            ...
            ShadowExecutionTotal = _meter.CreateCounter<long>("shadow_execution_total",
                description: "Total number of shadow engine executions");
            ShadowDriftedTotal = _meter.CreateCounter<long>("shadow_drifted_total",
                description: "Total number of shadow executions where drift was detected");
            ShadowCriticalDriftTotal = _meter.CreateCounter<long>("shadow_critical_drift_total",
                description: "Total number of critical shadow engine drifts");
            ShadowTraceMismatchTotal = _meter.CreateCounter<long>("shadow_trace_mismatch_total",
                description: "Total number of severe determinism violations (hash mismatch)");
            ShadowNonComparableTotal = _meter.CreateCounter<long>("shadow_non_comparable_total",
                description: "Total number of misalignment incidents (schema/trace incompatibility)");
            ...
            PostgresWalSyncDurationSeconds = _meter.CreateObservableGauge("postgres_wal_sync_duration_seconds",
                static () => _walSyncProvider(), unit: "s", description: "Latency of WAL sync operations from pg_stat_wal");
            PostgresDeadTupleRatio = _meter.CreateObservableGauge("postgres_dead_tuple_ratio",
                static () => _deadTupleRatioProvider(), description: "Ratio of dead vs live tuples in core ledger tables");
            PostgresCacheHitRatio = _meter.CreateObservableGauge("postgres_cache_hit_ratio",
                static () => _cacheHitRatioProvider(), description: "Efficiency of Postgres shared buffer cache");
            HangfireWaitToWorkRatio = _meter.CreateObservableGauge("hangfire_wait_to_work_ratio",
                static () => _waitToWorkRatioProvider(), description: "Ratio of job queued time vs execution time");
        ...
        public void RecordRecoveryRevenue(double amount, string currency) => RecoveryRevenueTotal.Add(amount, new KeyValuePair<string, object?>("currency", currency));

        public void RecordRecoveryDisputeStop(Guid customerId) => RecoveryDisputeStopTotal.Add(1, new KeyValuePair<string, object?>("customer_id", customerId.ToString()));
        ...
        public void RecordTimeToWow(double seconds, string outcomeCode) => TimeToWow.Record(seconds, new KeyValuePair<string, object?>("code", outcomeCode));

        public void RecordFeatureActivation(string featureName) => FeatureActivation.Add(1, new KeyValuePair<string, object?>("feature", featureName));
        ...
        public void RecordTimeToExpansion(double seconds) => TimeToExpansion.Record(seconds);
        ...
        private static CobryxModule GetModule(string code)
        {
            ...
            return Enum.TryParse<CobryxModule>(prefix, true, out var module) ? module : CobryxModule.Other;
        }
