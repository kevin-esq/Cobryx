...
public record GetDashboardSummaryQuery : IRequest<Result<DashboardSummaryDto>>, IRequiresTenant;
    ...
    public class GetDashboardSummaryQueryHandler(IUnitOfWork unitOfWork, ITenantProvider tenantProvider, IClock clock) : IRequestHandler<GetDashboardSummaryQuery, Result<DashboardSummaryDto>>
    {
        ...
        public async Task<Result<DashboardSummaryDto>> Handle(GetDashboardSummaryQuery request, CancellationToken cancellationToken)
        {
            ...
            var customersCount = await dbContext.Set<Domain.Lending.Customer>()
                .Where(c => c.TenantId == tenantId && !c.IsDeleted)
                .CountAsync(cancellationToken);

            var thirtyDaysAgo = _clock.UtcNow.AddDays(-30);
            var collectionsLast30d = await dbContext.Set<Domain.Payments.Payment>()
            ...
            var expectedCollectionsLast30d = await dbContext.Set<Installment>()
                .Where(i => i.Loan.TenantId == tenantId && !i.IsDeleted && i.DueDate >= thirtyDaysAgo && i.DueDate <= _clock.UtcNow)
